...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(windowX, windowY, "LearnOpenGL", nullptr, nullptr);
    if (window == nullptr)
    {
   ...
   /// glfwSetCursorPosCallback(window, mouse_callback);
    ...
    glfwSetCursorPos(window, windowX / 2, windowY / 2); //это сработало для управления тангажем
    /////////////
  ...
  // ------------------------------------------------------------------
    ...
    glm::vec3 fragpos = glm::vec3(0.5f, 0.5f, 0.5f);
    // glm::vec3 lightPos(1.2f, 1.0f, 2.0f);
    glm::vec3 lightDir = glm::normalize(lightPos - fragpos);
    normal = glm::normalize(normal);

    glm::vec3 viewDir = glm::normalize(camera.position_ - fragpos);
    ...
    float  spec = glm::pow(glm::max(glm::dot(viewDir, reflectDir), 0.0f), 64.0f) * 0.5f;

    std::cout << "lightDir.x " << lightDir.x << " lightDir.y " << lightDir.y << " lightDir.z " << lightDir.z << std::endl;
    std::cout << "normal.x " << normal.x << " normal.y " << normal.y << " normal.z " << normal.z << std::endl;
    std::cout << "dot diff" << diff << std::endl;
    std::cout << "dot spec " << spec << std::endl;
   ...
   // ------
        ...
        floor_shader.setInt("material.diffuse", 0);
        floor_shader.setVec3("viewPos", camera.position_);
        floor_shader.setVec3("light.direction", camera.front_);
        floor_shader.setVec3("light.position", camera.position_);

        floor_shader.setFloat("material.shininess", 32.0f);
        floor_shader.setVec3("light.ambient", 0.1f, 0.1f, 0.1f);
        floor_shader.setVec3("light.diffuse", 0.8f, 0.8f, 0.8f); // darken the light a bit to fit the scene
        floor_shader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);


        floor_shader.setFloat("light.cutOff", glm::cos(glm::radians(12.5f)));
        floor_shader.setFloat("light.outerCutOff", glm::cos(glm::radians(17.5f)));

        floor_shader.setFloat("light.constant", 1.0f);
        floor_shader.setFloat("light.linear", 0.09f);
        floor_shader.setFloat("light.quadratic", 0.032f);
